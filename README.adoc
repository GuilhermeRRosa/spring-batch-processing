...
- Utilizada a lib SpringBatch
- Criar ItemProcessor para tratar os dados (Conversão para UpperCase neste caso), input e output.
- Criar uma classe @configuration com as beans:
  - Reader
  - Processor
  - Writter
- Criar beans de configuração das Jobs:
  - Job (JobRepository, step1, JobCompletionNotificationListener)
  - Step1 (JobRepository, transactionManager, reader, processor, writer)
Criar classe JobCompletationNotificationListener
  - afterJob: configura o que será executado após o processo batch.
- SpringApplication.exit(): Garante que a aplicação seja terminada na JVM após sua execução.
